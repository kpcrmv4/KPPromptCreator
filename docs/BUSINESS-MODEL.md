...
|---|---|---|---|---|
| `spa` | SPA (single-page, fast) | +฿300 | → B | — |
| `pwa` | PWA + offline | +฿500 | → B | ต้องมี `spa` |
| `dark-mode` | Dark mode | +฿200 | — | — |
| `multi-lang` | Multi-language TH/EN | +฿400 | → B | — |
...
| `google-oauth` | Google OAuth | +฿300 | → B | ต้อง Google Cloud |
| `line-login` | LINE Login | +฿500 | → B | + LINE Login Channel |
| `email-password` | Email/Password | +฿700 | → B | + verify + reset |
| `phone-otp` | Phone OTP | +฿900 | → B | + SMS recurring (customer pay) |

### 4.4 Role/Permission

| Code | ชื่อ | Δ ราคา | Force Mode? |
|---|---|---|---|
| `role-2tier` | 2 roles (admin + user) | +฿300 | — |
| `role-custom` | Custom roles 3+ | +฿700 | — |
| `permission-row` | Permission per column/row | +฿1,000 | → B |
...
| `google-calendar` | Google Calendar sync | +฿500 | → B | Google Cloud |
...
| `webhook-out` | Webhook out (Zapier/Make) | +฿200 | — | Zapier URL |
| `sms-thai` | SMS (Thai SMS) | +฿600 | — | ThaiBulkSMS account |
