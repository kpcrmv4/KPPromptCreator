...
เริ่มเขียนไฟล์ "${fileName}" เลย:`;
    ...
    try {
        ...
        // Show codegen card and render estimate
        const codegenCard = document.getElementById('codegenCard');
        if (codegenCard) {
            codegenCard.style.display = 'block';
            renderCodegenEstimate(responseWithFingerprint);
        }
...
function switchMode(mode) {
    ...
    // Hide result on mode switch to prevent stale content
    const resultSection = document.getElementById('result-section');
    if (resultSection) {
        resultSection.style.display = 'none';
        const codegenCard = document.getElementById('codegenCard');
        if (codegenCard) codegenCard.style.display = 'none';
    }
