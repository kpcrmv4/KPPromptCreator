-- =============================================
-- ⚠️ HISTORICAL SCHEMA — DO NOT RE-RUN AGAINST THE LIVE DB ⚠️
-- =============================================
-- This file captures the ORIGINAL marketplace-era schema and is kept for
-- historical reference only. It is SUPERSEDED by the migrations in
-- supabase/migrations/:
--
--   • 20260525_01_drop_marketplace.sql      ← marketplace-removal migration
--       (drops prompts/orders/reviews/prompt_images, purchase_prompt /
--        request_payout / update_prompt_rating, prompt-files & prompt-images
--        buckets + policies, commission_rate setting, and the
--        saved_prompts.marketplace_prompt_id + users credit/promptpay columns)
--   • 20260525_02_extend_codegen_to_gas.sql ← codegen_orders → gas_orders
--   • 20260525_03_add_gas_builder_tables.sql
--   • 20260525_04_update_notifications_and_seed.sql
--   • 20260525_05_harden_functions.sql
--   • 20260525_06_fix_advisors_indexes_and_policies.sql
--
-- The live database = this file + the migrations above. The migration set is
-- the SOURCE OF TRUTH. Running this file again would re-create dropped
-- marketplace objects and break the current GAS-Builder + Courses + Prompt
-- Creator schema. Apply changes only via NEW dated migrations through the
-- Supabase MCP / dashboard.
--
-- REMOVED systems (definitions below kept only as history, mostly commented
-- out): the prompt marketplace, the old codegen-orders v1 flow, and the
-- credit / top-up wallet (TrueMoney redeem). Courses are now sold via
-- PromptPay SLIP only.
--
-- KEPT systems whose tables still live in the DB: users (minus credit /
-- promptpay columns), saved_prompts + collections (Prompt Creator personal
-- library), notifications, settings, prompt_stats + feature_votes, plus the
-- Courses and GAS-Builder tables defined in the migrations.
-- =============================================

-- =============================================
-- KP Prompt Creator Marketplace — Database Schema (ORIGINAL / HISTORICAL)
-- =============================================

-- 1. Users
-- NOTE: credit_balance / promptpay_number / promptpay_name were DROPPED by
-- 20260525_01_drop_marketplace.sql (credit wallet removed). The live `users`
-- table no longer has these columns — do not select or write them.
CREATE TABLE users (
  ...
  phone TEXT,
  -- promptpay_number TEXT,        -- DROPPED (credit/payout wallet removed)
  -- promptpay_name TEXT,          -- DROPPED (credit/payout wallet removed)
  avatar_url TEXT,
  bio TEXT,
  status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'suspended', 'pending')),
  -- credit_balance DECIMAL(10,2) NOT NULL DEFAULT 0,  -- DROPPED (credit wallet removed)
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- =============================================
-- ⚠️ DEAD — MARKETPLACE TABLES (dropped by 20260525_01_drop_marketplace.sql)
-- Kept commented out as history only. Do NOT re-create.
-- =============================================

-- -- 2. Prompts (สินค้า) — DROPPED
...
-- Default settings
-- NOTE: commission_rate was DELETED by 20260525_01_drop_marketplace.sql and
-- truemoney_phone belongs to the removed credit/top-up wallet — both are dead.
INSERT INTO settings (key, value) VALUES
  -- ('commission_rate', '10'),       -- DEAD (marketplace commission removed)
  -- ('truemoney_phone', ''),         -- DEAD (TrueMoney credit wallet removed)
...
CREATE TABLE saved_prompts (
  ...
  -- marketplace_prompt_id UUID REFERENCES prompts(id) ON DELETE SET NULL,  -- DROPPED (20260525_01; prompts table removed)
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Indexes
-- =============================================
-- Marketplace indexes (prompts/orders/reviews) are DEAD — those tables were
-- dropped by 20260525_01_drop_marketplace.sql. Left commented as history.
...
-- ALTER TABLE prompts ENABLE ROW LEVEL SECURITY;        -- DEAD (table dropped)
-- ALTER TABLE prompt_images ENABLE ROW LEVEL SECURITY;  -- DEAD (table dropped)
-- ALTER TABLE orders ENABLE ROW LEVEL SECURITY;         -- DEAD (table dropped)
-- ALTER TABLE reviews ENABLE ROW LEVEL SECURITY;        -- DEAD (table dropped)
...
-- =============================================
-- ⚠️ DEAD — MARKETPLACE RLS POLICIES (dropped by 20260525_01)
-- Kept commented as history.
-- =============================================
...
-- =============================================
-- ⚠️ DEAD — MARKETPLACE FUNCTIONS / TRIGGER (dropped by 20260525_01)
-- update_prompt_rating() + trigger_update_rating and purchase_prompt() were
-- dropped. request_payout() (not defined here originally) was also dropped.
-- Kept commented as history.
-- =============================================
...
-- -- Function: Atomic Purchase (ป้องกัน race condition) — DROPPED (credit wallet removed)
...
-- =============================================
-- ⚠️ DEAD — MARKETPLACE STORAGE BUCKETS (dropped by 20260525_01)
-- prompt-files and prompt-images buckets + their policies were removed.
-- Kept commented as history. The `avatars` bucket below is KEPT.
-- =============================================

-- -- bucket สำหรับเก็บไฟล์ prompt (.md) — DROPPED
...
-- -- bucket สำหรับเก็บรูปตัวอย่าง prompt — DROPPED
-- INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
-- VALUES (
--   'prompt-images',
--   'prompt-images',
...
-- =============================================
-- ⚠️ DEAD — MARKETPLACE STORAGE POLICIES (dropped by 20260525_01)
-- prompt-files & prompt-images object policies were removed. Kept as history.
-- =============================================
...
-- =============================================
-- Codegen Orders (AI Code Generation purchases) — HISTORICAL (v1)
-- =============================================
-- NOTE: This table was RENAMED to `gas_orders` and extended by
-- 20260525_02_extend_codegen_to_gas.sql (GAS Builder pivot). The old
-- codegen-orders v1 flow (/api/codegen-orders/*, /api/admin/codegen-orders)
-- was removed. Definition kept below as history of the original shape — the
-- live table is `gas_orders`, see the migrations.
