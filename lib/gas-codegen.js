...
- SpreadsheetApp.openById(SPREADSHEET_ID) for standalone web apps — use getActiveSpreadsheet() only if this is a container-bound script (script attached directly to a Google Sheet)
- SpreadsheetApp for database operations — always batch: getValues()/setValues() never getValue()/setValue() in a loop
- Use Named Ranges (ss.getRangeByName()) instead of hardcoded range strings like 'A2:F500'
- PropertiesService.getScriptProperties() for config/secrets — use getProperties()/setProperties() in batch
- LockService for concurrent access protection — always release in finally block; call SpreadsheetApp.flush() before releaseLock()
- CacheService for read-heavy data — use warmCache() in doGet() to avoid cold start; invalidate on write with cache.remove(key); avoid caching transactional data when writes are frequent
- Utilities.formatDate() for date handling with timezone
- Filter and transform data server-side before returning to client — never send raw 2D arrays; use indexed objects for O(1) lookups
- Lazy-load data via google.script.run after page load, or pre-render via HtmlService template (tmpl.data = getData())
