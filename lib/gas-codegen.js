...
- ALWAYS use SpreadsheetApp.getActiveSpreadsheet() to access the spreadsheet — NEVER use openById() or openByUrl() with hardcoded IDs
- If the project needs Google Docs, use DocumentApp.getActiveDocument() — NEVER hardcode document IDs
...
2. จะเปิดหน้าเว็บ installer ขึ้นมาอัตโนมัติ
3. เลือกวิธีติดตั้ง:
   - **Web App** — สร้างเว็บแอปเปิดผ่าน URL ได้เลย
   - **สร้าง Google Sheet + ผูกโค้ด** — สร้าง Sheet ใหม่พร้อมผูกโค้ดอัตโนมัติ
   - **ผูกกับ Sheet/Doc ที่มีอยู่** — ใส่ Script ID ของโปรเจกต์ที่มีอยู่
4. ทำตามขั้นตอนบนหน้าจอ (กดปุ่มเดียวทุกขั้นตอน)

### Mac / Linux
1. เปิด Terminal ในโฟลเดอร์นี้
2. รัน \`chmod +x setup.sh && ./setup.sh\`
3. จะเปิดหน้าเว็บ installer ขึ้นมา
4. เลือกวิธีติดตั้งและทำตามขั้นตอนบนหน้าจอ
`;
  }
